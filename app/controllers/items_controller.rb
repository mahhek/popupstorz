...
class ItemsController < ApplicationController
  ...
  def created_coming_gatherings
    ...
    @gatherings = @gatherings.uniq
    @gatherings = @gatherings.sort! { |g1, g2|  g1.rental_start_date <=> g2.rental_start_date }
  end
  ...
  def search_item_spaces
    ...
    respond_to do |format|
      ...
      format.js do
        foo = render_to_string(:partial => 'items', :locals => { :items => @items }).to_json
        render :js => "update_form_values('#{params.to_json}');$('#searched-items-div').html(#{foo});$.setAjaxPagination();"
      end
    end    
  end
