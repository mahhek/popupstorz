  ...
  def create
    ...
    if pay_response.success?
      ...
      if @offer.is_gathering or @offer.persons_in_gathering.to_i > 0
        ...
        if gathering_member.offer.user_id == current_user.id
          current_user.send_message(owner, :topic => t(:gather), :body => "#{current_user.first_name} #{t(:has_gather_for)} #{@offer.persons_in_gathering} #{t(:ppl_from)} #{@offer.rental_start_date.strftime("%b. %d, %Y")} #{t(:email_to)} #{@offer.rental_end_date.strftime("%b. %d, %Y")} #{t(:at_ur)} <a href='http://#{request.host_with_port}/items/#{@offer.item.id}'> #{@offer.item.title} </a> #{t(:and_now_waiting)}".html_safe)
          @notification = Notification.new(:user_id => owner.id, :notification_type => t(:gather), :description => "#{current_user.first_name} #{t(:has_gather_for)} #{@offer.persons_in_gathering} #{t(:ppl_from)} #{@offer.rental_start_date.strftime("%b. %d, %Y")} to #{@offer.rental_end_date.strftime("%b. %d, %Y")} #{t(:at_ur)} <a href='http://#{request.host_with_port}/items/#{@offer.item.id}'> #{@offer.item.title} </a> #{t(:and_now_waiting)}".html_safe)
          @notification.save
          flash[:notice] = t(:created_gathering)
        else
        ...
        unless @offer.offer_messages.last.message.blank?
        current_user.send_message(owner, :topic => t(:book_aprove_req), :body => "#{current_user.first_name} #{t(:would_let_rent)} <a href='http://#{request.host_with_port}/items/#{@offer.item.id}'> #{@offer.item.title} </a> #{t(:email_from)} #{@offer.rental_start_date.blank? ? '' : @offer.rental_start_date.strftime("%b. %d, %Y")} #{t(:email_to)}  #{@offer.rental_end_date.blank? ? '' : @offer.rental_end_date.strftime("%b. %d, %Y")} #{t(:need_responce_before)} #{@offer.cancellation_date.blank? ? '' : @offer.cancellation_date.strftime("%b. %d, %Y")}#{t(:go_to_listings)} #{current_user.first_name} #{t(:says2)}  #{@offer.offer_messages.last.message}.".html_safe)
        @notification = Notification.new(:user_id => owner.id, :notification_type =>t(:gather), :description => "#{current_user.first_name} #{t(:would_let_rent)} <a href='http://#{request.host_with_port}/items/#{@offer.item.id}'> #{@offer.item.title} </a> #{t(:email_from)} #{@offer.rental_start_date.blank? ? '' : @offer.rental_start_date.strftime("%b. %d, %Y")} #{t(:email_to)}  #{@offer.rental_end_date.blank? ? '' : @offer.rental_end_date.strftime("%b. %d, %Y")} #{t(:need_responce_before)} #{@offer.cancellation_date.blank? ? '' : @offer.cancellation_date.strftime("%b. %d, %Y")} #{t(:need_responce_before)} #{current_user.first_name} #{t(:says2)}  #{@offer.offer_messages.last.message}.".html_safe)
        else
          current_user.send_message(owner, :topic => t(:book_aprove_req), :body => "#{current_user.first_name} #{t(:would_let_rent)} <a href='http://#{request.host_with_port}/items/#{@offer.item.id}'> #{@offer.item.title} </a> #{t(:email_from)} #{@offer.rental_start_date.blank? ? '' : @offer.rental_start_date.strftime("%b. %d, %Y")} #{t(:email_to)}  #{@offer.rental_end_date.blank? ? '' : @offer.rental_end_date.strftime("%b. %d, %Y")} #{t(:need_responce_before)} #{@offer.cancellation_date.blank? ? '' : @offer.cancellation_date.strftime("%b. %d, %Y")}#{t(:go_to_listings)}".html_safe)
        @notification = Notification.new(:user_id => owner.id, :notification_type =>t(:gather), :description => "#{current_user.first_name} #{t(:would_let_rent)} <a href='http://#{request.host_with_port}/items/#{@offer.item.id}'> #{@offer.item.title} </a> #{t(:email_from)} #{@offer.rental_start_date.blank? ? '' : @offer.rental_start_date.strftime("%b. %d, %Y")} #{t(:email_to)}  #{@offer.rental_end_date.blank? ? '' : @offer.rental_end_date.strftime("%b. %d, %Y")} #{t(:need_responce_before)} #{@offer.cancellation_date.blank? ? '' : @offer.cancellation_date.strftime("%b. %d, %Y")} #{t(:need_responce_before)}".html_safe)
        end
        @notification.save
      ...
      session[:pay] = "new"
      redirect_to :action => "new?offer_id=#{@offer.id}&item_id=#{@item.id}"
    end
  end
