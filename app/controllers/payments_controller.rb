  ...
  def create
    ...
    if pay_response.success?
      ...
      if @offer.is_gathering or @offer.persons_in_gathering.to_i > 0
        ...
        current_user.send_message(owner, :topic => t(:book_aprove_req), :body => "#{current_user.first_name} #{t(:would_let_rent)} <a href='http://#{request.host_with_port}/items/#{@offer.item.id}'> #{@offer.item.title} </a> #{t(:email_from)} #{@offer.rental_start_date.blank? ? '' : @offer.rental_start_date.strftime("%m-%d-%Y")} #{t(:email_to)}  #{@offer.rental_end_date.blank? ? '' : @offer.rental_end_date.strftime("%m-%d-%Y")} #{t(:need_responce_before)} #{@offer.cancellation_date.blank? ? '' : @offer.cancellation_date.strftime("%m-%d-%Y")}#{t(:go_to_listing)} #{current_user.first_name} #{t(:says2)}  #{@offer.offer_messages.last.message}.".html_safe)
