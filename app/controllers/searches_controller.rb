class SearchesController < ApplicationController
  ...
  def search_gatherings
    ...
    @gatherings = []
    @items.each do|item|
      offers = item.offers.where("status = 'Applied' and parent_id is NULL and rental_start_date <= '#{Date.parse("#{Date.today}","%Y-%d-%m")}' and rental_end_date >= '#{Date.parse("#{Date.today}","%Y-%d-%m")}'")
